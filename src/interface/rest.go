...
import (
	...
	healthHandler "palm_code_be/src/interface/handlers/health"
	mediaHandler "palm_code_be/src/interface/handlers/media"
	pageHandler "palm_code_be/src/interface/handlers/page"
	teamHandler "palm_code_be/src/interface/handlers/team"
	uploadHandler "palm_code_be/src/interface/handlers/upload"
	userHandler "palm_code_be/src/interface/handlers/user"
	"palm_code_be/src/interface/response"
	"palm_code_be/src/interface/route"
...
func makeRoute(
	...
	corsMiddleware := cors.New(cors.Options{
		...
		MaxAge:           300, // maksimum umur (second)
	})
	...
	uph := uploadHandler.NewUploadHandler(respClient, useCases.UpLoadUC, useCases.MediaUC)
	ph := pageHandler.NewPageHandler(respClient, useCases.PageUC)
	mh := mediaHandler.NewMediaHandler(respClient, useCases.MediaUC)
	th := teamHandler.NewTeamHandler(respClient, useCases.TeamUC)
	r.Route("/api", func(r chi.Router) {
		r.Mount("/user", route.UserRouter(uh))
		r.Mount("/upload", route.UploadRouter(uph))
		r.Mount("/page", route.PageRouter(ph))
		r.Mount("/media", route.MediaRouter(mh))
		r.Mount("/team", route.TeamRouter(th))
	})
	return r
}
